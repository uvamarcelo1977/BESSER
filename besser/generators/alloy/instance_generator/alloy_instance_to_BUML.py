...
class AlloyToBUML:
    ...
    def __init__(self, xml_file: str, date_registry: DateRegistry | None = None):
        """
        Initializes the converter with the given Alloy XML file.

        Args:
            xml_file: Path to Alloy XML file
            date_registry: Registry mapping sequential ``dateN`` sig names to
                their ``dMMDDYYYY`` ids. When ``None`` (or empty) the converter
                falls back to decoding raw ``dMMDDYYYY`` literal ids from the
                atoms themselves.
        """
        self.date_registry = date_registry if date_registry is not None else DateRegistry.empty()
    ...
    def _date_signature_class_names(self) -> set[str]:
        ...
        signature, so they are excluded even when the ``DateRegistry`` is
        empty.
        """
        date_parent_ids = {
            sig_id
    ...
    def _date_value_expression(self, atom_label: str, as_datetime: bool) -> str:
        ...
        """
        if not as_datetime:
            return get_date_value(atom_label, self.date_registry)
        iso = get_date_value(atom_label, self.date_registry).strip('"')
    ...
    def is_date_value(self, atom_label: str) -> bool:
        ...
        """
        if self.date_registry.contains_atom(atom_label):
            return True
...
def get_date_value(atom_label: str, date_registry: DateRegistry | None = None) -> str:
    """
    Extracts the date value from an atom.

    Args:
        atom_label: atom label (e.g., 'date0$0', 'd01012000$0' or 'date$01')
        date_registry: Registry mapping sequential ``dateN`` sig names to their
            ``dMMDDYYYY`` ids. When ``None`` (or empty) the raw ``dMMDDYYYY``
            literal ids are decoded directly from the atom.

    Returns:
        Date value as an ISO-8601 string with quotes (e.g., '"2000-01-01"')
    """
    registry = date_registry if date_registry is not None else DateRegistry.empty()
    return registry.get_date_value(atom_label)
